...
133 IF PEEK(33021) <> 50 THEN PRINT"THIS VERSION REQUIRES A COCO 3":END
135 CLEAR 300,&H3DFF
140 CLS:PRINT@71,"D I S P L A Y - 3":PRINT@135,"BY DAVID MEREDITH":PRINT@201,"COPYRIGHT '83"
...
1210 I=INSTR(A$,QU$):IF I<>0 THEN J=INSTR(I+1,A$,QU$):IF J<>0 THEN NA$=MID$(A$,I,J-I+1) ELSE NA$=RIGHT$(A$,LEN(A$)-I) ELSE NA$=""
1220 IF LEN(NA$)>8 THEN NA$=LEFT$(NA$,8)
1230 LOADM NA$+".3D"
1240 GOTO 1800:REM DISPLAY PICTURE AFTER LOADING
1400 REM SAVE CURRENT PICTURE ON DISK
1410 I=INSTR(A$,QU$):IF I<>0 THEN J=INSTR(I+1,A$,QU$):IF J<>0 THEN NA$=MID$(A$,I,J-I+1) ELSE NA$=RIGHT$(A$,LEN(A$)-I) ELSE NA$=""
1420 IF LEN(NA$)>8 THEN NA$=LEFT$(NA$,8)
...
1820 FOR I=0 TO 25:CH(I)=0:NEXT
1830 I=LI
1840 P=PEEK(I):IF P<>255 THEN CH(P/9)=1:I=I+1:GOTO 1840:REM MARK ALL POINT NAMES USED
1850 FOR I=0 TO 25:IF CH(I)=0 THEN POKE PO+9*I,128:NEXT:REM MARK UNUSED POINTS AS UNDEFINED
1860 POKE 282,0:EXEC MA:POKE 282,255:PRINT:RETURN:REM TOGGLE LOWERCASE FLAG AND EXECUTE M/L ROUTINE
...
2140 VA=VAL(A$):IF ABS(VA)>80 THEN PRINT:PRINT"COORDINATES MUST BE BETWEEN -80 AND 80":PRINT:RETURN
2145 IF VA>=0 THEN V1=0:V2=VA ELSE V1=255:V2=256+VA
2150 POKE AD,V1:POKE AD+1,V2:POKE AD+2,0:RETURN
2200 REM PRINT LINES
2210 I=LI
2220 P=PEEK(I):IF P=255 THEN PRINT:RETURN
2230 PRINT CHR$(P/9+AA);CHR$(PEEK(I+1)/9+AA);" ";:I=I+2:GOTO 2220
2400 REM PRINT POINTS
2410 A1=0:A2=25:I=INSTR(A$,"-"):IF I=3 THEN A1=ASC(MID$(A$,2,1))-AA:IF A1<0 OR A1>25 THEN A1=0:GOTO 2030
2420 IF I<LEN(A$) AND I<=3 THEN A2=ASC(MID$(A$,I+1,1))-AA:IF A2<0 OR A2>25 THEN A2=25
2430 FOR I=A1 TO A2:AD=PO+9*I:IF PEEK(AD)=128 THEN NEXT:RETURN
2440 PRINT CHR$(I+AA);" = ";:FOR J=0 TO 6 STEP 3
2450 VA=PEEK(AD+J)*256+PEEK(AD+J+1)+PEEK(AD+J+2)/256:IF VA>32767 THEN VA=VA-65536
2460 V$=STR$(VA):PRINT V$;:IF J<>6 THENPRINT",";
